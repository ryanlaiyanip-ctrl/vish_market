...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Prevent resetting a resolved market — payouts already distributed
  if (market.status === "resolved") {
    return NextResponse.json({ error: "Cannot reset a resolved market — payouts have already been distributed" }, { status: 400 });
  }
